...
func (w *Worker) Find(email string) {
	for i := range w.users {
		...
		if strings.Contains(user.Email, email) {
			// log.Printf("found by worker %s -> ", w.name)
			w.ch <- user
		}
	}
}
...
func main() {
	...
	go NewWorker(DB[:6], ch, "#1").Find(email)
	go NewWorker(DB[6:12], ch, "#2").Find(email)
	go NewWorker(DB[12:], ch, "#3").Find(email)
	...
	for {
		...
		case <-time.After(1 * time.Second):
			// log.Printf("The email %s not found", email)
			return
		}
	}
}
